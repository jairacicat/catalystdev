...
* @Description Updates Revenue Plans using the Subscription Rev Rec rule to use the new Subscription rule
*
* CHANGELOGS
*
...
define(['N/file', 'N/search', 'N/record', 'N/runtime'], function(file, search, record, runtime) {
    ...
    function map(context) 
	{
        try
        {
            ...
            let revenuePlan = record.load({
                id: mapKey,
                type: record.Type.REVENUE_PLAN
            });

            revenuePlan.setValue({
                fieldId: 'revenuerecognitionrule',
                value: 14
            });

            revenuePlan.save({ignoreMandatoryFields: true})

            log.debug("REVENUEPLAN UPDATED", mapKey)
