...
define(['N/file', 'N/search', 'N/record', 'N/runtime'], function(file, search, record, runtime) {
    ...
    function map(context) 
	{
        try
        {
            ...
            let revenueArrangementId = mapValues.values["GROUP(revenuearrangement)"].value;
            let revenueArrangement = record.load({
                id: revenueArrangementId,
                type: record.Type.REVENUE_ARRANGEMENT
            });

            let lineCount = revenueArrangement.getLineCount({sublistId: 'revenueelement'});
            log.debug("Line Count", lineCount);

            for(var i = 0; i < lineCount; i++){
                let lineRevRecRule = revenueArrangement.getSublistValue({
                    sublistId: 'revenueelement',
                    line: i,
                    fieldId: 'revenuerecognitionrule'
                });

                let lineRevRecForecastRule = revenueArrangement.getSublistValue({
                    sublistId: 'revenueelement',
                    line: i,
                    fieldId: 'revrecforecastrule'
                });

                if(lineRevRecRule == 5 && lineRevRecForecastRule == 5){
            ...
            revenueArrangement.save({ignoreMandatoryFields: true});

            log.debug("REVENUE ARRANGEMENT UPDATED", revenueArrangementId)
