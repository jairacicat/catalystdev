    ...
    function(record, search, log, runtime, NSUtil, url, file) {
        ...
        function afterSubmit(scriptContext) {
            ...
            if (scriptContext.type != scriptContext.UserEventType.DELETE && scriptContext.type != scriptContext.UserEventType.PRINT) {
                ...
                var objTimeEntries = new Array();

                var timeSearchID = scriptObj.getParameter({
                    name: "custscript_time_saved_search"
                });
                ...
                //Check if Invoice has BillableTime
                var billableTimeCount = objInvoice.getLineCount({sublistId: 'time'});
                var billableTimeIds = new Array();
                if(billableTimeCount > 0){
                    for(var i = 0; i < billableTimeCount; i++){
                        var isApplied = objInvoice.getSublistValue({sublistId: 'time', fieldId: 'apply', line: i});
                        if(isApplied || isApplied == 'T'){
                            billableTimeIds.push(objInvoice.getSublistValue({sublistId: 'time', fieldId: 'doc', line: i}));
                        }  
                    } 
                }
                log.debug("billableTimeIds", billableTimeIds);
                if(billableTimeIds.length > 0){
                    var timeFilter = search.createFilter({
                        name: 'internalid',
                        operator: search.Operator.ANYOF,
                        values: billableTimeIds
                    });
                    timeSearch.filters.push(timeFilter);
                    log.debug("Time Search Filters", timeSearch.filters);
                    
                    timeSearch.run().each(function(result){
                        log.debug("timesearch - result", result);
                        objTimeEntries.push({
                            internalid: result.id,
                            "Date": strigyfyNull(result.getValue({
                                name: 'date'
                            ...
                            "Employee": strigyfyNull(result.getText({
                                name: 'employee'
                            })),
                            "Task": strigyfyNull(result.getText({
                                name: 'casetaskevent'
                ...
                log.debug("objTimeEntries", objTimeEntries);
                ...
                searchCharges.run().each(function(result) {
                    ...
                    */
                    ...
                    objInvoice.setValue('custbody_charge_list_milestone', JSON.stringify(objMilestoneCharges));
                    objInvoice.setValue('custbody_charge_list_purchase', JSON.stringify(objPurchaseCharges));
                    objInvoice.setValue('custbody_ctc_time_list', JSON.stringify(objTimeEntries));
