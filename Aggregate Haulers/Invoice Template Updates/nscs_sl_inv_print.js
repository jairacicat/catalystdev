    ...
    function (file, format, record, render, runtime, search, config,redirect,xml,task)  {
        ...
        function onRequest  (scriptContext) {
            try{
                ...
                var invSubsidiary = invRec.getValue('subsidiary');
                ...
                if(!isEmpty(soID)) {
                    ...
                    log.debug('ticketResults.length', ticketResults.length)
                        ...
                        try{
                            ...
                            if(err2.name==='FAILED_TO_SUBMIT_JOB_REQUEST_1'){
                                var html = '<!DOCTYPE html>'+'<html lang="en">'+'<head>'+'<meta charset="UTF-8">'+'<title>Scheduled Script Error</title>'+'</head>\n' +
                                    '<body>'+'<p><b>The Scheduled script to generate invoices is currently still running. Please wait to generate this invoice until after the script has completed execution. You can view the Scheduled Script status page here:</b></p>'+'<p width="80%">https://8152306.app.netsuite.com/app/common/scripting/scriptstatus.nl?daterange=ALL&datefrom=&dateto=&scripttype=1603&primarykey=2019&queueid=&jobstatefilterselect=&runtimeversion=&sortcol=dcreated&sortdir=DESC&csv=HTML&OfficeXML=F&pdf=&size=50&_csrf=pucgwXzcYCc0OmmIb6UIFsqPGDtF1aD1Z5AYtstNid-r89YdK</p>'+'</body>\n' +
                                    '</html>'
                            }else{
                                var html = '<!DOCTYPE html>'+'<html lang="en">'+'<head>'+'<meta charset="UTF-8">'+'<title>Scheduled Script Error</title>'+'</head>\n' +
                                    '<body>'+'<p><b>'+err2.message+'</b></p>'+'</body>\n' +
