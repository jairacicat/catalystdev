 ...
 * 1.02      Jan 31, 2024       asaliba             Added the function for the custom button to create the Retainer record
 * 1.02      Aug 26, 2024       karaneta            Added functionality for field change on update retainer field
 *
 */
...
define(['N/currentRecord', 'N/log', 'N/search', 'N/runtime', 'N/url', 'N/ui/message', 'N/https', 'SuiteScripts/CTC.Sentinel/CTC.SS2/CTC.Retainer/Retainer_Util.js'],
    /**
     * @param{currentRecord} currentRecord
     * @param log
     * @param search
     */
    function (currentRecord, log, search, runtime, url, message, https, rtnrutil) {
        ...
        function fieldChanged(scriptContext) {
            ...
            var createRetainer = current_rec.getField({fieldId: 'custbody_ctc_is_retainer_reference'});
            var updateRetainer = current_rec.getField({fieldId: 'custbody_ctc_update_retainer'});
            var parentRetainer = current_rec.getField({fieldId: 'custbody_ctc_rtnr_so_retainer'});
            var childRetainer = current_rec.getField({fieldId: 'custbody_ctc_rtnr_inv_retainer'});
            //log.debug(stLogTitle, 'fieldId: ' + fieldId);
            //console.log(stLogTitle, 'fieldId: ' + fieldId);

            if (fieldId === 'custbody_ctc_update_retainer') {
                var isUpdateRetainer = current_rec.getValue({
                    fieldId: 'custbody_ctc_update_retainer'
                });
                log.debug(stLogTitle, 'isUpdateRetainer ' + isUpdateRetainer);
                console.log(stLogTitle, 'isUpdateRetainer ' + isUpdateRetainer);
                ...
                if (isUpdateRetainer) {
                    ...
                    createRetainer.isDisabled = true;
                    parentRetainer.isDisabled = false;
                    childRetainer.isDisabled = true;
                } else {
                    createRetainer.isDisabled = false;
                    parentRetainer.isDisabled = true;
                    parentRetainer.isDisabled = false;
                }
            ...
            if (fieldId === 'custbody_ctc_is_retainer_reference') {
                ...
                if (isRetainer) {
                    ...
                    updateRetainer.isDisabled = true;
                    parentRetainer.isDisabled = true;
                    current_rec.setValue({
                        fieldId: 'custbody_ctc_update_retainer',
                        value: false
        ...
        function saveRecord(scriptContext) {
            ...
            if (!rtnrutil.isEmpty(retainerId) || isRetainerReference) {
                var itemId = null;
                for (var i = 0; i <= itemCount - 1; i++) {
                    ...
                    itemId = current_rec.getCurrentSublistValue({
                        sublistId: 'item',
                        fieldId: 'item'
                    });
        ...
        function createRetainer(recId) {

            try {

                var messageProcessing = message.create({
                    title: 'Creating Retainer',
                    type: message.Type.INFORMATION
                });
                messageProcessing.show();

                var suiteletURL = url.resolveScript({
                    scriptId: 'customscript_ctc_sl_so_retainer',
                    deploymentId: 'customdeploy_ctc_sl_so_retainer',
                    params: {
                        'recId': recId
                    }
                });
                ...
                var response = https.post.promise({
                    ...
                    .then(function (response) {
                        ...
                        messageProcessing.hide();

                        var messageComplete = message.create({
                            title: 'Retainer created',
                ...
                log.error('Error - createRetainer', error.message);
            }
        }
