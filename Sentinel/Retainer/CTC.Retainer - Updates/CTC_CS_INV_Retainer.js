 ...
 * 1.02      Feb 15, 2023       karaneta            Show pop up and change content.
 * 1.03      Aug 30, 2024       karaneta            remove pop up for update retainer
 * 1.04      Sept 4, 2024       karaneta            reinstate pop up for update retainer
 * 1.05      Sept 12, 2024      karaneta            add exclusion for add to retainer pop up
 */
    ...
    function (currentRecord, log, search, rtnrutil) {
        ...
        function pageInit(scriptContext) {
            ...
            if (!rtnrutil.isEmpty(current_recId)) {
                ...
                addToRetainerField = current_rec.getField({
                ...
                parentRetainer = current_rec.getValue({
            ...
            updateRetainer = current_rec.getValue({
                fieldId: 'custbody_ctc_update_retainer'
            });

            addToRetainer = current_rec.getValue({
                fieldId: 'custbody_ctc_inv_addto_retainer'
            });

            log.debug(stLogTitle, 'updateRetainer: ' + updateRetainer + ' |addToRetainer: ' + addToRetainer);
            ...
            if (stContext === 'copy' || stContext === 'edit') {
                ...
                if (isRetainerReference) {
                    ...
                    addToRetainerField.isDisabled = true;
        ...
        function saveRecord(scriptContext) {
            ...
            //1.02
            if (!updateRetainer && !addToRetainer) {
                if (retainerItemFound === true && !retainerId) {
                    log.debug(stLogTitle, 'itemId == retainerItemId && !retainerId');

                    alert('Please select a Retainer record');
                    return false;
