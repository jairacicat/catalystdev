...
define(['N/file', 'N/render', 'N/search', 'N/log', 'N/redirect', 'N/record',
    ...
    function(file, render, search, log, redirect, record, config, serverWidget) {
        ...
        function getCustomerRetainers(customerId) {
            ...
            var pagedData = retainerSearch.runPaged({
                pageSize: 1000
            });
                ...
                try {
                    ...
                    if (!isEmpty(custObj)) {
                        for (var i = 0; i < custObj.length; i++) {
                            var entityID = custObj[i].getValue({
                                name: 'entityid'
                            });
                            var at = custObj[i].getValue({
                                name: 'attention'
                            });
                ...
                try {
                    ...
                    if (!isEmpty(invoiceResult)) {
                        for (var i = 0; i < invoiceResult.length; i++) {
                            var inv = invoiceResult[i].getValue({
                                name: 'tranid',
                                summary: "GROUP"
                            });
                ...
                try {
                    ...
                    if (!isEmpty(creditMemoResult)) {
                        for (var i = 0; i < creditMemoResult.length; i++) {
                            var cm = creditMemoResult[i].getValue({
                                name: 'tranid',
                                summary: "GROUP"
                            });
                ...
                try {
                    ...
                    if (!isEmpty(retainerResult)) {
                        for (var i = 0; i < retainerResult.length; i++) {
                            var retName = retainerResult[i].getValue({
                                name: 'name'
                            });
                            var startDate = retainerResult[i].getValue({
                                name: 'custrecord_ctc_rtnr_start_date'
                            });
                            var endDate = retainerResult[i].getValue({
                                name: 'custrecord_ctc_rtnr_end_date'
                            });
                            var totalBudget = retainerResult[i].getValue({
                                name: 'custrecord_ctc_rtnr_total_budget'
                            });
                            var retainerType = retainerResult[i].getText({
                                name: 'custrecord_ctc_rtnr_type'
                            }); //ALI
                            //var beginningBalance = retainerResult[i].getValue({name: 'custrecord_ctc_rtnr_beginningbal'}); //ALI
                            var retainerBudegetRemaining = retainerResult[i].getValue({
                                name: 'custrecord_ctc_rtnr_total_bgt_remaining'
                            });
                            var retainerDateCreated = retainerResult[i].getValue({
                                name: 'created'
                            });
        ...
        function getRetainerBeginningBudget(retainerId) {
            ...
            customrecord_ctc_retainerSearchObj.run().each(function(result) {
                if (result.getValue({
                    ...
                    beginningBudget = result.getValue({
                        name: 'oldvalue',
                        join: "systemNotes",
                        summary: "GROUP"
                    });
