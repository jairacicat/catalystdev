 ...
 * 1.10     Aug 27, 2024    karaneta        Add update parent retainer functionality
...
define(['N/record', 'N/search', 'N/log', 'N/runtime', 'N/ui/serverWidget', 'SuiteScripts/CTC.Sentinel/CTC.SS2/CTC.Retainer/Retainer_Util.js'], function (record, search, log, runtime, serverWidget, rtnrutil) {
    ...
    function rtnr_afterSubmit(context) {
        ...
        if (context.type === context.UserEventType.CREATE || context.type === context.UserEventType.EDIT) {
            ...
            var parentRetainer = current_rec.getValue({
                fieldId: 'custbody_ctc_rtnr_so_retainer'
            });
            ...
            log.debug(stLogTitle, 'updateRetainer: ' + updateRetainer + ' |parentRetainer:' + parentRetainer);

            if (updateRetainer && !rtnrutil.isEmpty(parentRetainer)) {
                //get retainer amount
                ...
                var updatePTStat = rtnrutil.updateParentRetainer(parentRetainer, retainerAmount, customerId);
                log.audit(stLogTitle, 'updatePTStat:' + updatePTStat);
    ...
    function rtnr_afterSubmit_old(context) {
        log.audit('Start rtnr_afterSubmit', new Date());
        var stLogTitle = 'rtnr_afterSubmit';
        log.debug(stLogTitle, '-------------> SCRIPT ENTRY <------------------');
        log.debug(stLogTitle, 'CONTEXT ' + JSON.stringify(context));
