...
//CWM Parameters
var CWM_NAME = nlapiGetContext().getSetting('SCRIPT', 'custscript_ctc_oip_cfg_cwmname');
var CWM_API_MEMBER_NAME = nlapiGetContext().getSetting('SCRIPT', 'custscript_ctc_oip_cfg_apimember');
var CWM_TIME_ENTRY_CU_TASK_ID_FIELD_ID = 40; //TODO: Change in PRODUCTION
var CWM_TIME_ENTRY_CU_TIME_ID_FIELD_ID = 41; //TODO: Change in PRODUCTION
var CWM_TICKET_CU_TASK_LINK_ID_FIELD_ID = 38; //TODO: Change in PRODUCTION
var CWM_TICKET_CU_TASK_ID_FIELD_ID = 39; //TODO: Change in PRODUCTION
var CW_SCHEDULE_TYPE_ID = 4; //TODO: Change in PRODUCTION
//CLICKUP Parameters
var CLICKUP_NAME = nlapiGetContext().getSetting('SCRIPT', 'custscript_ctc_oip_cfg_cuname');
//var CU_TEAMSPACE_ID = nlapiGetContext().getSetting('SCRIPT', 'custscript_ctc_oip_cfg_spaceid');
var CU_AUTH_ACCESS_TOKEN = nlapiGetContext().getSetting('SCRIPT', 'custscript_ctc_oip_cfg_token');
//var CU_CLIENT_ID = nlapiGetContext().getSetting('SCRIPT', 'custscript_ctc_oip_cfg_cuclientid');
var CU_API_URL = 'https://api.clickup.com';
var CU_BASE_URL = 'https://app.clickup.com';
var CU_SPACE_ID = nlapiGetContext().getSetting('SCRIPT', 'custscript_ctc_oip_cfg_spaceid');
var CW_ACCOUNT_ID = nlapiGetContext().getSetting('SCRIPT', 'custscript_ctc_oip_cfg_cwaccount');
//var CW_TICKET_REDIRECT_URL = 'https://na.myconnectwise.net/v4_6_release/services/system_io/router/openrecord.rails?locale=en-US&companyName=nscatalyst&recordType=ServiceFV&recid='; //TODO: FOR PRODUCTION
var CW_TICKET_REDIRECT_URL = nlapiGetContext().getSetting('SCRIPT', 'custscript_ctc_oip_cfg_cwmredirecturl');
var CU_SYSTEM_INTEGRATION_ID = nlapiGetContext().getSetting('SCRIPT', 'custscript_ctc_oip_cfg_cuintegrationid');

//CWM Parameters
// var CWM_NAME = 'Connectwise Manage';
// var CWM_API_MEMBER_NAME = 'ClickUpv2';
// var CWM_TIME_ENTRY_CU_TASK_ID_FIELD_ID = 40; //TODO: Change in PRODUCTION
// var CWM_TIME_ENTRY_CU_TIME_ID_FIELD_ID = 41; //TODO: Change in PRODUCTION
// var CWM_TICKET_CU_TASK_LINK_ID_FIELD_ID = 38; //TODO: Change in PRODUCTION
// var CWM_TICKET_CU_TASK_ID_FIELD_ID = 39; //TODO: Change in PRODUCTION
// var CW_SCHEDULE_TYPE_ID = 4; //TODO: Change in PRODUCTION
// //CLICKUP Parameters
// var CLICKUP_NAME = 'Clickup';
// //var CU_TEAMSPACE_ID = nlapiGetContext().getSetting('SCRIPT', 'custscript_ctc_oip_cfg_spaceid');
// var CU_AUTH_ACCESS_TOKEN = '32389598_87bae635f77d023dcb40caadabd812db5e9fffa1';
// //var CU_CLIENT_ID = nlapiGetContext().getSetting('SCRIPT', 'custscript_ctc_oip_cfg_cuclientid');
// var CU_API_URL = 'https://api.clickup.com';
// var CU_BASE_URL = 'https://app.clickup.com';
// var CU_SPACE_ID = '36066827';
// var CW_ACCOUNT_ID = '17';
// //var CW_TICKET_REDIRECT_URL = 'https://na.myconnectwise.net/v4_6_release/services/system_io/router/openrecord.rails?locale=en-US&companyName=nscatalyst&recordType=ServiceFV&recid='; //TODO: FOR PRODUCTION
// var CW_TICKET_REDIRECT_URL = 'https://na.myconnectwise.net/v4_6_release/services/system_io/router/openrecord.rails?locale=en-US&companyName=nscatalyst&recordType=ServiceFV&recid=';
// var CU_SYSTEM_INTEGRATION_ID = '32389598';
    ...
    if(request.getMethod() == 'POST') {
        if (request.getParameter('custpage_activatewebhook') == 'T') {
            ...
            var jsonPostObjectWH = {
                "id": 3,
                "url": "https://tstdrv1716438.extforms.netsuite.com/app/site/hosting/scriptlet.nl?script=309&deploy=1&compid=TSTDRV1716438&h=619d07712869fc7a359a&origin=cwm",
                "objectId": 1,
                "type": "ticket",
                "level": "owner",
                "inactiveFlag": false
                    ...
                    if (historyItemsArr[0].after) {
                        ...
                        if (userId != CU_SYSTEM_INTEGRATION_ID) { //if the callback came from system integration user - ignore these, only real people task updates should be recognized
                            ...
                            if(duplicateDetected == true) {
                                initialLogStatus = INTEGRATION_STATUS_REJECTED;
                                messageLog = messageLog + '\n' + nlapiDateToString(new Date(), 'datetime') + ': INVALID: Duplicate time entry detected on CWM. Rejecting time entry creation webhook.';
                            }
                            ...
                            if (taskTimeObject.description && taskTimeObject.description != '' && taskTimeObject.description != 'undefined' && duplicateDetected == false) {
                                createCWMTimeEntry(cwTicketId, JSONData.task_id, timeMemberId, taskObject.companyid, timeId, timeStart, timeEnd, taskTimeObject.description, logRecordId, timeSeconds, taskObject.isprojectticket);
                                customerFlagAlreadyUpdated = true;
                            }
...
function createCWMTimeEntry(cwTicketId, cuTaskId, memberId, companyId, timeId, timeStart, timeEnd, description, logRecordId, timeSeconds, isProjectTicket) {
    ...
    try {
        ...
        if(cwTicketId && timeId && timeStart) {
            ...
            updateLog(logRecordId, returnStatus, messageLog, CW_TICKET_REDIRECT_URL + cwTicketId, JSON.stringify(JSONPostObject), destinationURL);
        }
    } catch (err) {
    ...
    try {
        ...
        if(taskId != '' && taskId) {
            ...
            if (JSONPostObject7b) {
             ...
             nlapiLogExecution("Emergency", "Customer Type",  "start");
             var JSONPostObjectCustType = {};
             var customerType = '';
             nlapiLogExecution("debug", "JSONData",  JSON.stringify(JSONData));
             nlapiLogExecution("debug", "JSONData company",  JSON.stringify(JSONData.company));

             if(JSONData.company) {
                companyId = JSONData.company.id;
                customerType = getCompanyType(companyId);
                nlapiLogExecution("Emergency", "customerTYpe", JSON.stringify(customerType));
            }
            JSONPostObjectCompType.value = customerType;
            nlapiLogExecution("Emergency", "JSONPostObjectCompType",  JSONPostObjectCompType.value);
            var customFieldId_customerType = getCUCustomFieldId('CW Customer Type', cuTaskJSONData);
            nlapiLogExecution("Emergency", "customFieldId_customerType",  "customFieldId_customerType");
            if (JSONPostObjectCompType) {
                messageLog = messageLog + '\n\n' + nlapiDateToString(new Date(), 'datetime') + ': HTTP PUT UPDATE DATA OBJECT - CUSTOM FIELD COMPANY ID UPDATE: ' + JSON.stringify(JSONPostObjectCustType);
                nlapiLogExecution('AUDIT', 'HTTP PUT UPDATE DATA OBJECT - CUSTOM FIELD CUSTOMER TYPE UPDATE:', JSON.stringify(JSONPostObjectCustType));
                var postRequestCustType = nlapiRequestURL(CU_API_URL + '/api/v2/task/' + taskId + '/field/' + customFieldId_customerType, JSON.stringify(JSONPostObjectCustType), getCUHeaderSettings(), 'POST');
                nlapiLogExecution("Emergency", "postRequestCustType",  "postRequestCustType");
            ...
            nlapiLogExecution("Emergency", "Customer Type",  "end");
