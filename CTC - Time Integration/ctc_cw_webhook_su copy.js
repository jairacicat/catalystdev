...
var CW_TIME_REDIRECT_URL = 'https://na.myconnectwise.net/v2024_1/services/system_io/router/openrecord.rails?locale=en-US&companyName=nscatalyst&recordType=TimeEntryFV&recid=';
...
function customRound(value) {
    ...
    if (decimalPart >= 0.99) {
      return wholePart + 1; // Rounds up if the decimal part is 0.99 or higher
    } else {
      // Returns the number with its specific decimal part if it's .25, .5, .75
      // For other decimals, it will just return the original value
      return value;
    }
  }
...
function updateCWMTimeEntry(cwTicketId, cuTaskId, memberId, companyId, timeId, timeStart, timeEnd, description, logRecordId, timeSeconds, isProjectTicket, isChargeCode, cwmTimeId) {
    ...
    try {
        ...
        if(cwTicketId && timeId && timeStart) {
            ...
            var JSONPostObject2 = cwmTimeObject;
            JSONPostObject2["customFields"] =[
                {
                    "id": CWM_TIME_ENTRY_CU_TASK_ID_FIELD_ID,
                    "caption": "CU Task ID",
                    "numberOfDecimals": 0,
            ...
            JSONPostObject2["actualHours"] = customRound(actualHours);
            nlapiLogExecution('EMERGENCY', "CUSTOM ROUND ACTUAL HOURS", customRound(actualHours));
            JSONPostObject2["notes"] = description;
            ...
            nlapiLogExecution('EMERGENCY', 'JSON POST for CW Account ID: ' + CW_ACCOUNT_ID, JSON.stringify(JSONPostObject2));
            //POST
            //messageLog = messageLog + '\n\n' + nlapiDateToString(new Date(), 'datetime') + ': HTTP POST DATA OBJECT: ' + JSON.stringify(postDataObject);
            postRequest = postJSONData('time/entries/' + cwmTimeId, JSONPostObject2, null, 'PUT', CW_ACCOUNT_ID);
            ...
            var postRequestBody = JSON.parse(postRequest.getBody());
            destinationURL = CW_TIME_REDIRECT_URL + postRequestBody.id;
            updateLog(logRecordId, returnStatus, messageLog, String(postRequestBody.id), JSON.stringify(JSONPostObject2), destinationURL);
        }
    } catch (err) {
        messageLog = messageLog + '\n' + nlapiDateToString(new Date(), 'datetime') + ': [ERROR]: Could not post time entry on CWM for ticket id: ' + cwTicketId + ' REASON: ' + err;
        updateLog(logRecordId, INTEGRATION_STATUS_ERROR, messageLog, '', JSON.stringify(JSONPostObject));
